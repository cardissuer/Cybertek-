...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
        String [] invalidWords = {"a", "an", "the","and", "but", "for", "nor", "or", "so", "yet", "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
        List<String> listOfWords = Arrays.asList(invalidWords);
    ...
    public void setAuthor(String author) {
        ...
        for(String word: author.trim().split(" ")){

            fixedAuthor += capitalize(word) + " ";

        }

        this.author = fixedAuthor.trim();
    ...
    public void setTitle(String title) {
        ...
        String [] allWords = title.trim().split(" ");
        String fixedTitle = "";
        fixedTitle = capitalize(allWords[0]);

        if(allWords.length != 1) {

            for(int i=1; i < allWords.length; i++) {

                if(isTitleWord(allWords[i])) {
                    fixedTitle += capitalize(allWords[i]) + " ";
                } else {
                    fixedTitle += allWords[i].toLowerCase() + " ";
                }
    ...
    public String toString() {
        ...
        return "[author=" + getAuthor() + ", title=" + getTitle() + ", id=" + getId() + ", pages=" + getPages() + "]";
